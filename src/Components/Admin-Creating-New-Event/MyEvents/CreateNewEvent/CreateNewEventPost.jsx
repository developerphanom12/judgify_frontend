...
export const CreateNewEventPost = () => {
  ...
  const handleIndustryChange = (industry) => {
    if (formData.industry_type.includes(industry)) {

      setFormData({
        ...formData,
        industry_type: formData.industry_type.filter((item) => item !== industry),
      });
    } else {

      setFormData({
        ...formData,
        industry_type: [...formData.industry_type, industry],
      });
    }
  ...
  console.log(formData.additional_email, "111")
                        ...
                        {isDropdownVisible && (
                          <div className="dropdown_list">
                            {IndustryTypes.map((industry, index) => (
                              <div
                                key={index}
                                className={`dropdown-option ${formData.industry_type.includes(industry) ? "selected" : ""}`}
                                onClick={() => handleIndustryChange(industry)}
                                style={{
                                  cursor: "pointer",
                                  backgroundColor: formData.industry_type.includes(industry) ? "#e0e0e0" : "white",
                                  pointerEvents: formData.industry_type.includes(industry) ? "none" : "auto",
                                }}
                              >
                                {industry}
                              </div>
